...
// OpenGL/Vulkan graphics context creation, etc.) (Requires: GLFW 3.0+. Prefer GLFW 3.3+/3.4+ for
...
//  cursor shape and visibility (ImGuiBackendFlags_HasMouseCursors) with GLFW 3.1+. Resizing cursors
//  requires GLFW 3.4+! Disable with 'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. [X]
//  Multiple Dear ImGui contexts support.
// Missing features or Issues:
//  [ ] Platform: Touch events are only correctly identified as Touch on Windows. This create issues
//  with some interactions. GLFW doesn't provide a way to identify touch inputs from mouse inputs,
//  we cannot call io.AddMouseSourceEvent() to identify the source. We provide a Windows-specific
//  workaround. [ ] Platform: Missing ImGuiMouseCursor_Wait and ImGuiMouseCursor_Progress cursors.
...
//  2026-04-21: Added a Win32-specific implementation of ImGui_ImplGlfw_GetContentScaleXXXX
//  functions for legacy GLFW 3.2. 2026-03-25: Mouse cursor is properly restored if changed by user
//  app/code while using glfwSetInputMode(..., GLFW_CURSOR_DISABLED) or
//  ImGuiConfigFlags_NoMouseCursorChange. Amend change from 2025-12-10. 2026-02-10: Try to set
//  IMGUI_IMPL_GLFW_DISABLE_X11 / IMGUI_IMPL_GLFW_DISABLE_WAYLAND automatically if corresponding
//  headers are not accessible. (#9225) 2025-12-12: Added IMGUI_IMPL_GLFW_DISABLE_X11 /
//  IMGUI_IMPL_GLFW_DISABLE_WAYLAND to forcefully disable either. 2025-12-10: Avoid repeated
//  glfwSetCursor()/glfwSetInputMode() calls when unnecessary. Lowers overhead for very high
//  framerates (e.g. 10k+ FPS). 2025-11-06: Lower minimum requirement to GLFW 3.0. Though a recent
//  version e.g GLFW 3.4 is highly recommended. 2025-09-18: Call platform_io.ClearPlatformHandlers()
//  on shutdown. 2025-09-15: Content Scales are always reported as 1.0 on Wayland. FramebufferScale
//  are always reported as 1.0 on X11. (#8920, #8921) 2025-07-08: Made
...
#include <GLFW/glfw3native.h>
#elif GLFW_HAS_X11
#ifndef GLFW_EXPOSE_NATIVE_X11 // for glfwGetX11Display(), glfwGetX11Window() on Freedesktop (Linux,
                               // BSD, etc.)
...
static bool ImGui_ImplGlfw_IsWayland()
{
    ...
    if (strstr(version, "Wayland") ==
        nullptr) // e.g. Ubuntu 22.04 ships with GLFW 3.3.6 compiled without Wayland
        return false;
                                ...
                                GlfwClientApi client_api)
{
    ...
    snprintf(bd->BackendPlatformName, sizeof(bd->BackendPlatformName), "imgui_impl_glfw (%d)%s",
             GLFW_VERSION_COMBINED, bd->IsWayland ? " (Wayland)" : "");
    ...
    // Emscripten: the same application can run on various platforms, so we detect the Apple
    // platform at runtime to override io.ConfigMacOSXBehaviors from its default (which is always
    // false in Emscripten).
#ifdef __EMSCRIPTEN__
#if EMSCRIPTEN_USE_PORT_CONTRIB_GLFW3 >= 34020240817
    if (emscripten::glfw3::IsRuntimePlatformApple())
    {
        ...
        // Due to how the browser (poorly) handles the Meta Key, this line essentially disables
        // repeats when used. This means that Meta + V only registers a single key-press, even if
        // the keys are held. This is a compromise for dealing with this issue in ImGui since ImGui
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ...
    {
        bd->LastMouseCursor = nullptr; // Invalidate so that if user changes underlying cursor we
                                       // will update it next time we can.
        return;
    }
...
// For GFLW 3.2 + Windows: include a simplified non-monitor aware version of
// ImGui_ImplWin32_GetDpiScaleForMonitor(). This is merely a band-aid to make using GLFW 3.2 a
// little bit nicer, but prefer to use GLFW 3.3+ or the full correct functions from the Win32
// backend.
#if !GLFW_HAS_PER_MONITOR_DPI && defined(_WIN32) && !defined(NOGDI)
static float ImGui_ImplWin32_GetLegacyDpiScale()
{
...
static void glfwGetWindowContentScale(GLFWwindow*, float* x_scale, float* y_scale)
{
    *x_scale = *y_scale = ImGui_ImplWin32_GetLegacyDpiScale();
}
static void glfwGetMonitorContentScale(GLFWmonitor*, float* x_scale, float* y_scale)
{
    *x_scale = *y_scale = ImGui_ImplWin32_GetLegacyDpiScale();
}
...
void ImGui_ImplGlfw_InstallEmscriptenCallbacks(GLFWwindow*, const char* canvas_selector)
{
    ...
    // We intentionally do not check 'if (install_callbacks)' here, as some users may set it to
    // false and call GLFW callback themselves.
